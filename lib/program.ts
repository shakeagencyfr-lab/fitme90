...
Pour la musculation : cardio:false avec sets et reps normaux. REPOS : renseigne "restSec" (repos par défaut de la séance, en secondes) ET, pour CHAQUE exercice de musculation, un "rest" en secondes adapté au cycle. RÈGLE DE STYLE : n'utilise JAMAIS de tiret cadratin (—) ni de tiret demi-cadratin (–) dans les textes ; écris avec une ponctuation naturelle (virgules, deux-points, points, parenthèses). ADRESSE : tous les textes lus par le client ("summary", "body" de chaque cycle, "warning", "note" des exercices) sont écrits par le coach QUI PARLE AU CLIENT, à la deuxième personne du singulier (« tu », « ton genou », « tes séances »). Jamais à la troisième personne : pas de « le client », « il », « elle », ni de portrait du type « Prénom, 39 ans, danseuse de formation ». Tu peux dire « Prénom, on construit… » puis continuer en « tu ». Le programme est rédigé dans la langue demandée plus haut ; en anglais, la même règle vaut avec « you ».`;
