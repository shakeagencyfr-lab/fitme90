...
import { FOODS, RECIPES, type Food, type Ingredient, type Repas, type RecipeTemplate, type Allergene, type Regime } from "@/lib/recipe-catalog";
...
export function partsPour(repas: readonly Repas[]): Record<string, number> {
  const collation = repas.includes("collation");
  const part = collation ? 0.15 : 0;
  const reste = 1 - part;
  const out: Record<string, number> = {};
  for (const r of repas) out[r] = r === "collation" ? part : PART_BASE[r] * reste;
  return out;
}
...
export function scaleRecipe(tpl: RecipeTemplate, cible: Macros): ScaledRecipe {
  ...
  const ingredients = tpl.ingredients.map((i) => {
    ...
    return { food: i.food, nom: food.nom, grammes: g, libelle: libelleQuantite(food, g) };
  });
...
export function buildMenu(plan: PlanRepas): ScaledRecipe[] {
  ...
  for (const repas of plan.repas) {
    const pool = RECIPES.filter((r) => r.repas === repas && convient(r, plan.profil) && !deja.has(r.id));
    if (pool.length === 0) continue;

    // Tri stable et déterministe : score décroissant, puis identifiant.
    const trie = [...pool].sort((a, b) => {
      const d = scoreRecette(b, plan.profil) - scoreRecette(a, plan.profil);
...
export function repasDuJour(a: Record<string, unknown>): Repas[] {
  ...
  return Number.isFinite(n) && n >= 4 ? [...base, "collation"] : base;
}
