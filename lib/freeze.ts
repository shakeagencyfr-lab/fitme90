...
export async function tenantFreezeState(tenantId: string | null): Promise<FreezeState> {
  if (!tenantId) return { frozen: false, status: null };
  const admin = createAdminClient();
  const { data } = await admin
    .from("tenants")
    .select("sub_id, sub_status")
    .eq("id", tenantId)
    .maybeSingle<{ sub_id: string | null; sub_status: string | null }>();
  const status = data?.sub_status ?? null;
  const frozen = !!data?.sub_id && !!status && FROZEN_STATUSES.has(status);
