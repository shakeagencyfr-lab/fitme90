    ...
    <div className="rl-float rounded-[26px] border border-white/12 bg-white/[0.04] p-4 shadow-[0_30px_80px_-24px_rgba(0,0,0,.7)] backdrop-blur-xl">
      <div className="rounded-[18px] bg-[#0c0f12] p-4">
        ...
        {kind === "ai" ? (
          ...
          <div>
            <div className="grid grid-cols-3 gap-2">{[["Prot.", "156 g"], ["Gluc.", "210 g"], ["Lip.", "62 g"]].map(([k, v]) => (<div key={k} className="rounded-xl border border-white/10 bg-white/[0.03] p-2.5 text-center"><div className="font-archivo text-[15px] font-extrabold text-white">{v}</div><div className="text-[10px] text-white/45">{k}</div></div>))}</div>
            <div className="mt-3 flex items-center gap-3 rounded-xl border border-brand/25 bg-brand/[0.08] p-3"><span className="flex size-9 items-center justify-center rounded-lg bg-brand/20 text-brand"><LIcon name="nutrition" className="h-5 w-5" /></span><div className="text-[12.5px] text-white/85">Bowl poulet & patate douce<br /><span className="text-white/45">642 kcal · 12 min</span></div></div>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div id="top" className="relative min-h-dvh overflow-hidden scroll-smooth bg-[#080a0c] pb-[76px] text-white [scrollbar-color:#333_#080a0c] sm:pb-0" style={{ ["--color-brand" as string]: accent, ["--color-brand-hover" as string]: `color-mix(in srgb, ${accent} 85%, #000)` } as CSSProperties}>
      ...
      <header className="relative z-30 sticky top-0 border-b border-white/10 bg-[#080a0c]/70 backdrop-blur-xl">
        <div className="mx-auto flex w-full max-w-[1160px] items-center justify-between gap-4 px-5 py-3.5 sm:px-8">
          ...
          <div className="flex items-center gap-2">
            <Link href="/connexion" className="hidden text-[14px] font-medium text-white/60 transition-colors hover:text-white sm:inline">Connexion</Link>
      ...
      <section className="relative z-10 mx-auto grid w-full max-w-[1160px] items-center gap-12 px-5 py-16 sm:px-8 sm:py-24 lg:grid-cols-[1.05fr_0.95fr]">
        <div>
          <span className="rl-up inline-flex items-center gap-2 rounded-pill border border-white/15 bg-white/5 px-3.5 py-1.5 text-[12.5px] font-medium text-white/75"><span className="size-1.5 rounded-full bg-brand" style={{ animation: "rlPulse 2s ease-in-out infinite" }} /> Ta web app de coaching · Boostée par l&apos;IA</span>
          <h1 className="rl-up mt-5 font-archivo text-[clamp(34px,6.5vw,60px)] font-extrabold leading-[1.02] tracking-[-0.035em]" style={{ animationDelay: "80ms" }}>
            <span className="bg-gradient-to-br from-white to-white/70 bg-clip-text text-transparent">Ta web app de coaching,</span> <span className="text-brand">prête à vendre.</span>
          </h1>
          <p className="rl-up mt-5 max-w-[56ch] text-[16.5px] leading-[1.7] text-white/70" style={{ animationDelay: "160ms" }}>Lance ton activité de coaching sous ta marque, propulsée par l&apos;IA. Programmes, nutrition, chat, paiements : tout est prêt. Toi, tu vends. <span className="text-white/90">Ton premier client est offert.</span></p>
          <div className="rl-up mt-8 flex flex-wrap items-center gap-3" style={{ animationDelay: "240ms" }}>
            <Link href={signup} className="tap group inline-flex items-center justify-center gap-2 rounded-btn bg-brand px-7 py-4 text-[15.5px] font-semibold text-white shadow-[0_10px_40px_-8px_var(--color-brand)] transition-[transform,background-color] hover:bg-brand-hover active:scale-[0.98]">Créer mon espace coach <LIcon name="arrow" className="h-4 w-4 transition-transform group-hover:translate-x-0.5" /></Link>
          ...
          <div className="rl-up mt-7 flex flex-wrap items-center gap-x-5 gap-y-2 text-[13px] text-white/55" style={{ animationDelay: "320ms" }}>
            {["Premier client offert", "Aucune ligne de code", "Sans engagement"].map((t) => (<span key={t} className="inline-flex items-center gap-1.5"><LIcon name="check" className="h-4 w-4 text-brand" /> {t}</span>))}
          </div>
        </div>
        <div className="rl-up relative mx-auto w-full max-w-[380px]" style={{ animationDelay: "220ms" }}>
          <div className="rl-float rounded-[28px] border border-white/12 bg-white/[0.04] p-4 shadow-[0_30px_80px_-20px_rgba(0,0,0,.7)] backdrop-blur-xl">
            <div className="rounded-[20px] bg-[#0c0f12] p-4">
              ...
              <div className="mt-4 rounded-2xl border border-white/10 bg-white/[0.03] p-3.5"><div className="flex items-center gap-2 text-[11px] font-mono uppercase tracking-[0.12em] text-white/45"><span className="flex size-6 items-center justify-center rounded-full bg-brand/20 text-brand"><LIcon name="ai" className="h-3.5 w-3.5" /></span>Coach IA</div><p className="mt-2 text-[13px] leading-[1.5] text-white/85">Ta séance du jour est prête 💪 On vise +2 reps sur le développé. Prêt ?</p></div>
              <div className="mt-3 rounded-2xl border border-brand/25 bg-brand/[0.08] p-3.5"><div className="flex items-center justify-between"><span className="text-[12px] font-semibold text-white/90">Programme · Cycle 2 · Jour 24</span><span className="font-archivo text-[13px] font-bold text-brand">On track</span></div><div className="mt-2.5 h-1.5 w-full overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-brand" style={{ width: "73%" }} /></div></div>
              <div className="mt-3 grid grid-cols-3 gap-2">{[["Force", "+18%"], ["Séances", "21"], ["Assiduité", "94%"]].map(([k, v]) => (<div key={k} className="rounded-xl border border-white/10 bg-white/[0.03] p-2.5 text-center"><div className="font-archivo text-[15px] font-extrabold text-white">{v}</div><div className="text-[10px] text-white/45">{k}</div></div>))}</div>
            </div>
          </div>
          <div className="pointer-events-none absolute -bottom-5 -left-5 rounded-2xl border border-white/12 bg-[#0c0f12]/90 px-4 py-3 shadow-xl backdrop-blur-xl" style={{ animation: "rlFloat 6s ease-in-out infinite", animationDelay: "1.5s" }}><div className="flex items-center gap-2.5"><span className="flex size-9 items-center justify-center rounded-full bg-brand/15 text-brand"><LIcon name="card" className="h-5 w-5" /></span><div><div className="font-archivo text-[15px] font-extrabold leading-none text-white">+1 abonné</div><div className="mt-0.5 text-[11px] text-white/50">revenu récurrent</div></div></div></div>
        </div>
      </section>

      {/* Marquee */}
      <div className="relative z-10 overflow-hidden border-y border-white/10 bg-white/[0.02] py-4"><div className="rl-marquee flex w-max items-center gap-4">{[...MARQUEE, ...MARQUEE].map((m, i) => (<span key={i} className="inline-flex items-center gap-4 text-[15px] font-semibold text-white/45"><span className="size-1.5 rounded-full bg-brand" /> {m}</span>))}</div></div>
      ...
      <section id="apercu" className="relative z-10 border-y border-white/10 bg-white/[0.02]">
        <div className="mx-auto flex w-full max-w-[1160px] flex-col gap-16 px-5 py-20 sm:px-8 sm:py-28 sm:gap-24">
          <Reveal className="mx-auto max-w-[680px] text-center"><span className="font-mono text-[11px] uppercase tracking-[0.18em] text-brand">L&apos;aperçu</span><h2 className="mt-4 font-archivo text-[clamp(26px,4.5vw,42px)] font-extrabold leading-[1.08] tracking-[-0.025em]">Une expérience premium, à ta marque</h2></Reveal>
          {SHOWCASE.map((s, i) => (
            <div key={s.kind} className={`grid items-center gap-10 lg:grid-cols-2 ${i % 2 === 1 ? "lg:[&>*:first-child]:order-2" : ""}`}>
              ...
              <Reveal delay={100}><span className="font-mono text-[11px] uppercase tracking-[0.16em] text-brand">{s.tag}</span><h3 className="mt-3 font-archivo text-[clamp(22px,3.5vw,32px)] font-extrabold leading-[1.1] tracking-[-0.02em]">{s.title}</h3><p className="mt-4 text-[15.5px] leading-[1.7] text-white/65">{s.desc}</p><ul className="mt-5 flex flex-col gap-2.5">{s.points.map((p) => (<li key={p} className="flex items-center gap-2.5 text-[14.5px] text-white/80"><LIcon name="check" className="h-4 w-4 shrink-0 text-brand" /> {p}</li>))}</ul></Reveal>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="simulateur" className="relative z-10 mx-auto w-full max-w-[1160px] px-5 py-20 sm:px-8 sm:py-28">
        <Reveal className="mx-auto max-w-[680px] text-center"><span className="font-mono text-[11px] uppercase tracking-[0.18em] text-brand">Simulateur</span><h2 className="mt-4 font-archivo text-[clamp(26px,4.5vw,42px)] font-extrabold leading-[1.08] tracking-[-0.025em]">Combien peux-tu gagner ?</h2><p className="mt-4 text-[16px] leading-[1.7] text-white/65">Fais glisser les curseurs : ton nombre de clients, ton tarif. Ton revenu récurrent s&apos;affiche en direct.</p></Reveal>
      ...
      <section className="relative z-10 border-y border-white/10 bg-white/[0.02]">
        <div className="mx-auto w-full max-w-[1160px] px-5 py-20 sm:px-8 sm:py-28">
          <Reveal className="max-w-[620px]"><span className="font-mono text-[11px] uppercase tracking-[0.18em] text-brand">Clé en main</span><h2 className="mt-4 font-archivo text-[clamp(26px,4.5vw,42px)] font-extrabold leading-[1.08] tracking-[-0.025em]">Tout ce qu&apos;il te faut, déjà prêt</h2><p className="mt-4 text-[16px] leading-[1.7] text-white/65">Une plateforme complète. Toi, tu gères tes clients. Le reste tourne tout seul.</p></Reveal>
          <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{FEATURES.map((f, i) => (<Reveal key={f.title} delay={(i % 3) * 80} className="group rounded-[22px] border border-white/10 bg-gradient-to-b from-white/[0.04] to-transparent p-6 transition-all hover:-translate-y-1 hover:border-brand/40"><span className="flex size-12 items-center justify-center rounded-2xl bg-brand/12 text-brand transition-colors group-hover:bg-brand/20"><LIcon name={f.icon} className="h-6 w-6" /></span><h3 className="mt-4 font-archivo text-[18px] font-bold">{f.title}</h3><p className="mt-2 text-[14px] leading-[1.6] text-white/60">{f.desc}</p></Reveal>))}</div>
        </div>
      </section>

      {/* Étapes */}
      <section className="relative z-10 mx-auto w-full max-w-[1160px] px-5 py-20 sm:px-8 sm:py-28">
        <Reveal className="text-center"><span className="font-mono text-[11px] uppercase tracking-[0.18em] text-brand">En 3 étapes</span><h2 className="mx-auto mt-4 max-w-[620px] font-archivo text-[clamp(26px,4.5vw,42px)] font-extrabold leading-[1.08] tracking-[-0.025em]">De zéro à ton premier client aujourd&apos;hui</h2></Reveal>
        <div className="relative mt-12 grid gap-4 md:grid-cols-3"><div className="pointer-events-none absolute left-[16%] right-[16%] top-9 hidden h-px bg-gradient-to-r from-brand/50 via-brand/20 to-brand/50 md:block" />{STEPS.map((s, i) => (<Reveal key={s.n} delay={i * 100} className="relative rounded-[22px] border border-white/10 bg-[#0c0f12] p-7"><span className="font-archivo text-[42px] font-extrabold leading-none text-brand/25">{s.n}</span><h3 className="mt-3 font-archivo text-[19px] font-bold">{s.title}</h3><p className="mt-2 text-[14.5px] leading-[1.6] text-white/60">{s.desc}</p></Reveal>))}</div>
      </section>

      {/* Pour qui */}
      <section className="relative z-10 mx-auto w-full max-w-[1160px] px-5 py-16 sm:px-8 sm:py-20">
        <Reveal className="text-center"><span className="font-mono text-[11px] uppercase tracking-[0.18em] text-brand">Pour qui</span><h2 className="mx-auto mt-4 max-w-[560px] font-archivo text-[clamp(24px,4vw,36px)] font-extrabold tracking-[-0.02em]">Pensé pour tous ceux qui vendent du résultat</h2></Reveal>
        <div className="mt-9 flex flex-wrap justify-center gap-3">{SECTORS.map((s, i) => (<Reveal key={s.label} delay={i * 60} className="inline-flex items-center gap-2.5 rounded-pill border border-white/12 bg-white/[0.03] px-4 py-2.5 text-[14px] font-medium text-white/80"><span className="text-brand"><LIcon name={s.icon} className="h-5 w-5" /></span>{s.label}</Reveal>))}</div>
      </section>

      {/* Tarifs */}
      <section id="formules" className="relative z-10 border-y border-white/10 bg-white/[0.02]">
        <div className="mx-auto w-full max-w-[1160px] px-5 py-20 sm:px-8 sm:py-28">
          <Reveal className="text-center"><span className="font-mono text-[11px] uppercase tracking-[0.18em] text-brand">Tarifs</span><h2 className="mx-auto mt-4 max-w-[640px] font-archivo text-[clamp(26px,4.5vw,42px)] font-extrabold leading-[1.08] tracking-[-0.025em]">Ton premier client est <span className="text-brand">offert</span></h2><p className="mx-auto mt-4 max-w-[52ch] text-[16px] leading-[1.6] text-white/65">Lance ton activité sans rien payer. Tu passes à une formule seulement quand tu accueilles ton deuxième client.</p></Reveal>
          {plans.length > 0 ? (
            <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{plans.map((p, i) => (<Reveal key={p.id} delay={i * 80} className="flex flex-col gap-4 rounded-[24px] border border-white/12 bg-gradient-to-b from-white/[0.05] to-white/[0.01] p-7 transition-all hover:-translate-y-1 hover:border-brand/40"><div className="font-archivo text-[20px] font-bold">{p.name}</div><div><span className="font-archivo text-[30px] font-extrabold tracking-[-0.02em] text-brand">{priceLine(p) || "Sur mesure"}</span></div><div className="text-[14px] text-white/70">{p.client_limit == null ? "Clients illimités" : `Jusqu'à ${p.client_limit} client${p.client_limit > 1 ? "s" : ""} actifs`}</div>{p.setup_fee_cents > 0 ? <div className="text-[12.5px] text-white/45">+ {formatEuros(p.setup_fee_cents)} de mise en place (une fois)</div> : null}<Link href={signup} className="tap mt-auto inline-flex items-center justify-center rounded-btn bg-brand px-5 py-3.5 text-[14.5px] font-semibold text-white transition-[transform,background-color] hover:bg-brand-hover active:scale-[0.98]">Commencer</Link></Reveal>))}</div>
          ) : (
            <Reveal className="mx-auto mt-12 max-w-[480px] rounded-[24px] border border-brand/25 bg-gradient-to-b from-brand/[0.12] to-transparent p-9 text-center"><div className="mx-auto flex size-14 items-center justify-center rounded-2xl bg-brand/15 text-brand"><LIcon name="bolt" className="h-7 w-7" /></div><div className="mt-4 font-archivo text-[26px] font-extrabold">Premier client offert</div><p className="mt-2 text-[14.5px] leading-[1.6] text-white/65">Crée ton espace et démarre gratuitement. Tu passes à une formule quand tu grandis.</p><Link href={signup} className="tap mt-6 inline-flex items-center justify-center gap-2 rounded-btn bg-brand px-7 py-4 text-[15px] font-semibold text-white transition-[transform,background-color] hover:bg-brand-hover active:scale-[0.98]">Créer mon espace coach <LIcon name="arrow" className="h-4 w-4" /></Link></Reveal>
          )}
          <Reveal delay={120} className="mx-auto mt-10 flex max-w-[720px] flex-wrap items-center justify-center gap-x-7 gap-y-3 text-[13.5px] text-white/60">{["Sans carte bancaire", "Sans engagement", "Annule quand tu veux", "Données hébergées en UE"].map((t) => (<span key={t} className="inline-flex items-center gap-1.5"><LIcon name="check" className="h-4 w-4 text-brand" /> {t}</span>))}</Reveal>
        </div>
      </section>

      {/* FAQ */}
      <section id="faq" className="relative z-10 mx-auto w-full max-w-[820px] px-5 py-20 sm:px-8 sm:py-28">
        <Reveal className="text-center"><span className="font-mono text-[11px] uppercase tracking-[0.18em] text-brand">Questions</span><h2 className="mt-4 font-archivo text-[clamp(26px,4.5vw,40px)] font-extrabold tracking-[-0.025em]">Tout ce que tu te demandes</h2></Reveal>
        <div className="mt-10 flex flex-col gap-3">{FAQ.map((item, i) => (<Reveal key={item.q} delay={i * 50}><details className="group rounded-[18px] border border-white/10 bg-white/[0.03] px-5 py-4 transition-colors open:border-brand/30"><summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-[15.5px] font-semibold text-white/90 [&::-webkit-details-marker]:hidden">{item.q}<span className="flex size-7 shrink-0 items-center justify-center rounded-full border border-white/15 text-white/60 transition-transform group-open:rotate-45"><svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M12 5v14M5 12h14" /></svg></span></summary><p className="mt-3 text-[14.5px] leading-[1.7] text-white/60">{item.a}</p></details></Reveal>))}</div>
      </section>

      {/* CTA final */}
      <section className="relative z-10 mx-auto w-full max-w-[1160px] px-5 pb-20 sm:px-8">
        <Reveal className="relative overflow-hidden rounded-[28px] border border-white/12 p-10 text-center sm:p-16"><div className="pointer-events-none absolute inset-0" style={{ background: `radial-gradient(90% 120% at 50% 0%, color-mix(in srgb, ${accent} 30%, transparent), transparent 70%)` }} /><div className="relative"><h2 className="mx-auto max-w-[680px] font-archivo text-[clamp(28px,5vw,46px)] font-extrabold leading-[1.05] tracking-[-0.03em]">Ton business de coaching commence maintenant</h2><p className="mx-auto mt-4 max-w-[52ch] text-[16px] leading-[1.6] text-white/70">Ton premier client est offert. Aucune carte requise. Sois en ligne dans 5 minutes.</p><Link href={signup} className="tap mt-8 inline-flex items-center justify-center gap-2 rounded-btn bg-brand px-8 py-4 text-[16px] font-semibold text-white shadow-[0_12px_44px_-8px_var(--color-brand)] transition-[transform,background-color] hover:bg-brand-hover active:scale-[0.98]">Créer mon espace coach <LIcon name="arrow" className="h-4 w-4" /></Link></div></Reveal>
      </section>
      ...
      <footer className="relative z-10 border-t border-white/10">
        <div className="mx-auto flex w-full max-w-[1160px] flex-col gap-6 px-5 py-12 sm:flex-row sm:items-center sm:justify-between sm:px-8">
          <div className="flex flex-col gap-2"><Link href="#top" className="text-white [&_span]:text-white"><Wordmark size={20} /></Link><p className="max-w-[42ch] text-[12.5px] leading-[1.6] text-white/45">La plateforme qui transforme les coachs et salles en marques de coaching digitales, propulsées par l&apos;IA.</p></div>
          <nav className="flex flex-wrap items-center gap-x-6 gap-y-2 text-[13.5px]">
            <Link href="/revendeurs" className="font-semibold text-brand transition-opacity hover:opacity-80">Programme revendeur</Link>
            <Link href="/inscription-coach" className="text-white/60 transition-colors hover:text-white">Créer mon espace</Link>
            <Link href="/connexion" className="text-white/60 transition-colors hover:text-white">Connexion</Link>
            <Link href="/cgv" className="text-white/60 transition-colors hover:text-white">CGV</Link>
            <Link href="/confidentialite" className="text-white/60 transition-colors hover:text-white">Confidentialité</Link>
          </nav>
        </div>
      </footer>

      {/* CTA collante mobile */}
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-white/10 bg-[#080a0c]/92 px-4 py-3 backdrop-blur-xl sm:hidden"><Link href={signup} className="tap flex w-full items-center justify-center gap-2 rounded-btn bg-brand py-3.5 text-[15px] font-semibold text-white active:scale-[0.98]">Démarrer gratuitement <LIcon name="arrow" className="h-4 w-4" /></Link></div>
    </div>
  );
}
