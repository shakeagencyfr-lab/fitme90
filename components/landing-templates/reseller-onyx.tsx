          ...
          <div>
            <div className="grid grid-cols-3 gap-2">
              {[["Prot.", "156 g"], ["Gluc.", "210 g"], ["Lip.", "62 g"]].map(([k, v]) => (
                <div key={k} className="rounded-xl border border-white/10 bg-white/[0.03] p-2.5 text-center"><div className="font-archivo text-[15px] font-extrabold text-white">{tx(v)}</div><div className="text-[10px] text-white/45">{tx(k)}</div></div>
              ))}
            </div>
            <div className="mt-3 flex items-center gap-3 rounded-xl border border-brand/25 bg-brand/[0.08] p-3">
              ...
              <div className="text-[12.5px] text-white/85">{tx("Bowl poulet & patate douce")}<br /><span className="text-white/45">{tx("642 kcal · 12 min")}</span></div>
            </div>
          </div>
        )}
      </div>
...
export function ResellerOnyx({ reseller, plans, freePlan = null }: { reseller: PublicReseller; plans: Plan[]; freePlan?: Plan | null }) {
    ...
    >
        ...
        <div className="mx-auto flex w-full max-w-[1160px] items-center justify-between gap-4 px-5 py-3.5 sm:px-8">
          ...
          <nav className="hidden items-center gap-7 md:flex">
            {[["#apercu", "Aperçu"], ["#simulateur", "Simulateur"], ["#formules", "Tarifs"], ["#faq", "FAQ"]].map(([href, label]) => (
              <a key={href} href={href} className="text-[14px] font-medium text-white/60 transition-colors hover:text-white">{tx(label)}</a>
            ))}
          </nav>
      ...
      <section className="relative z-10 mx-auto grid w-full max-w-[1160px] items-center gap-12 px-5 py-16 sm:px-8 sm:py-24 lg:grid-cols-[1.05fr_0.95fr]">
        ...
        <div className="rl-up relative mx-auto w-full max-w-[380px]" style={{ animationDelay: "220ms" }}>
          <div className="rl-float rounded-[28px] border border-white/12 bg-white/[0.04] p-4 shadow-[0_30px_80px_-20px_rgba(0,0,0,.7)] backdrop-blur-xl">
            <div className="rounded-[20px] bg-[#0c0f12] p-4">
              ...
              <div className="mt-4 rounded-2xl border border-white/10 bg-white/[0.03] p-3.5">
                <div className="flex items-center gap-2 text-[11px] font-mono uppercase tracking-[0.12em] text-white/45"><span className="flex size-6 items-center justify-center rounded-full bg-brand/20 text-brand"><Ic name="ai" className="h-3.5 w-3.5" /></span>{tx("Coach IA")}</div>
                <p className="mt-2 text-[13px] leading-[1.5] text-white/85">{tx("Ta séance du jour est prête 💪 On vise +2 reps sur le développé. Prêt ?")}</p>
              </div>
              <div className="mt-3 rounded-2xl border border-brand/25 bg-brand/[0.08] p-3.5">
                <div className="flex items-center justify-between"><span className="text-[12px] font-semibold text-white/90">{tx("Programme · Cycle 2 · Jour 24")}</span><span className="font-archivo text-[13px] font-bold text-brand">{tx("On track")}</span></div>
          ...
          <div className="pointer-events-none absolute -bottom-5 -left-5 rounded-2xl border border-white/12 bg-[#0c0f12]/90 px-4 py-3 shadow-xl backdrop-blur-xl" style={{ animation: "rlFloat 6s ease-in-out infinite", animationDelay: "1.5s" }}>
            <div className="flex items-center gap-2.5"><span className="flex size-9 items-center justify-center rounded-full bg-brand/15 text-brand"><Ic name="card" className="h-5 w-5" /></span><div><div className="font-archivo text-[15px] font-extrabold leading-none text-white">{tx("+1 abonné")}</div><div className="mt-0.5 text-[11px] text-white/50">{tx("revenu récurrent")}</div></div></div>
          </div>
        </div>
      </section>
      ...
      <section className="relative z-10 mx-auto grid w-full max-w-[1160px] grid-cols-2 gap-6 px-5 py-14 sm:grid-cols-4 sm:px-8 sm:py-16">
        {[
          ...
          <Reveal key={tx(s.l)} delay={i * 90} className="text-center sm:text-left">
            ...
            <div className="mt-1 text-[13.5px] text-white/55">{tx(s.l)}</div>
          </Reveal>
        ))}
      </section>
      ...
      <section className="relative z-10 mx-auto w-full max-w-[1160px] px-5 py-16 sm:px-8 sm:py-24">
        <Reveal className="mx-auto max-w-[680px] text-center">
          <span className="font-mono text-[11px] uppercase tracking-[0.18em] text-brand">{tx("Le déclic")}</span>
          <h2 className="mt-4 font-archivo text-[clamp(26px,4.5vw,42px)] font-extrabold leading-[1.08] tracking-[-0.025em]">{tx("Change de dimension, sans changer de métier")}</h2>
        </Reveal>
        <div className="mt-12 grid gap-4 md:grid-cols-2">
          <Reveal className="rounded-[24px] border border-white/10 bg-white/[0.02] p-7">
            <div className="inline-flex items-center gap-2 rounded-pill border border-white/10 px-3 py-1 text-[12px] font-semibold text-white/50">{tx("Sans plateforme")}</div>
            <ul className="mt-5 flex flex-col gap-3.5">
              {COMPARE_WITHOUT.map((t) => (
                <li key={t} className="flex items-start gap-3 text-[14.5px] leading-[1.5] text-white/55">
                  <span className="mt-0.5 flex size-5 shrink-0 items-center justify-center rounded-full bg-white/10 text-white/50"><Ic name="x" className="h-3 w-3" /></span>{tx(t)}
                </li>
              ))}
            </ul>
          </Reveal>
      ...
      <section id="apercu" className="relative z-10 border-y border-white/10 bg-white/[0.02]">
        <div className="mx-auto flex w-full max-w-[1160px] flex-col gap-16 px-5 py-20 sm:px-8 sm:py-28 sm:gap-24">
          <Reveal className="mx-auto max-w-[680px] text-center">
            <span className="font-mono text-[11px] uppercase tracking-[0.18em] text-brand">{tx("L'aperçu")}</span>
            <h2 className="mt-4 font-archivo text-[clamp(26px,4.5vw,42px)] font-extrabold leading-[1.08] tracking-[-0.025em]">{tx("Une expérience premium, à ta marque")}</h2>
          </Reveal>
          {SHOWCASE.map((s, i) => (
            <div key={s.kind} className={`grid items-center gap-10 lg:grid-cols-2 ${i % 2 === 1 ? "lg:[&>*:first-child]:order-2" : ""}`}>
              ...
              <Reveal delay={100}>
                <span className="font-mono text-[11px] uppercase tracking-[0.16em] text-brand">{tx(s.tag)}</span>
                <h3 className="mt-3 font-archivo text-[clamp(22px,3.5vw,32px)] font-extrabold leading-[1.1] tracking-[-0.02em]">{tx(s.title)}</h3>
                <p className="mt-4 text-[15.5px] leading-[1.7] text-white/65">{tx(s.desc)}</p>
                <ul className="mt-5 flex flex-col gap-2.5">
                  {s.points.map((p) => (
                    <li key={p} className="flex items-center gap-2.5 text-[14.5px] text-white/80"><Ic name="check" className="h-4 w-4 shrink-0 text-brand" /> {tx(p)}</li>
                  ))}
                </ul>
              </Reveal>
            </div>
      ...
      <section id="simulateur" className="relative z-10 mx-auto w-full max-w-[1160px] px-5 py-20 sm:px-8 sm:py-28">
        <Reveal className="mx-auto max-w-[680px] text-center">
          ...
          <h2 className="mt-4 font-archivo text-[clamp(26px,4.5vw,42px)] font-extrabold leading-[1.08] tracking-[-0.025em]">{tx("Combien peux-tu gagner ?")}</h2>
          <p className="mt-4 text-[16px] leading-[1.7] text-white/65">{tx("Fais glisser les curseurs : ton nombre de clients, ton tarif. Ton revenu récurrent s'affiche en direct.")}</p>
        </Reveal>
      ...
      <section className="relative z-10 mx-auto w-full max-w-[1160px] px-5 py-20 sm:px-8 sm:py-28">
        <Reveal className="max-w-[620px]">
          ...
          <h2 className="mt-4 font-archivo text-[clamp(26px,4.5vw,42px)] font-extrabold leading-[1.08] tracking-[-0.025em]">{tx("Et tout le reste, déjà prêt")}</h2>
          <p className="mt-4 text-[16px] leading-[1.7] text-white/65">{tx("Une plateforme complète. Toi, tu gères tes clients. Le reste tourne tout seul.")}</p>
        </Reveal>
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {FEATURES.map((f, i) => (
            <Reveal key={f.title} delay={(i % 3) * 80} className="group rounded-[22px] border border-white/10 bg-gradient-to-b from-white/[0.04] to-transparent p-6 transition-all hover:-translate-y-1 hover:border-brand/40">
              ...
              <h3 className="mt-4 font-archivo text-[18px] font-bold">{tx(f.title)}</h3>
              <p className="mt-2 text-[14px] leading-[1.6] text-white/60">{tx(f.desc)}</p>
            </Reveal>
          ))}
        </div>
      </section>
      ...
      <section className="relative z-10 border-y border-white/10 bg-white/[0.02]">
        <div className="mx-auto w-full max-w-[1160px] px-5 py-20 sm:px-8 sm:py-28">
          <Reveal className="text-center">
            ...
            <h2 className="mx-auto mt-4 max-w-[620px] font-archivo text-[clamp(26px,4.5vw,42px)] font-extrabold leading-[1.08] tracking-[-0.025em]">{tx("De zéro à ton premier client aujourd'hui")}</h2>
          </Reveal>
          <div className="relative mt-12 grid gap-4 md:grid-cols-3">
            ...
            {STEPS.map((s, i) => (
              <Reveal key={s.n} delay={i * 100} className="relative rounded-[22px] border border-white/10 bg-[#0c0f12] p-7">
                ...
                <h3 className="mt-3 font-archivo text-[19px] font-bold">{tx(s.title)}</h3>
                <p className="mt-2 text-[14.5px] leading-[1.6] text-white/60">{tx(s.desc)}</p>
              </Reveal>
            ))}
          </div>
        </div>
      ...
      <section className="relative z-10 mx-auto w-full max-w-[1160px] px-5 py-16 sm:px-8 sm:py-20">
        <Reveal className="text-center">
          <span className="font-mono text-[11px] uppercase tracking-[0.18em] text-brand">{tx("Pour qui")}</span>
          <h2 className="mx-auto mt-4 max-w-[560px] font-archivo text-[clamp(24px,4vw,36px)] font-extrabold tracking-[-0.02em]">{tx("Pensé pour tous ceux qui vendent du résultat")}</h2>
        </Reveal>
      ...
      <PricingProvider plans={plans}>
        <div className="mx-auto w-full max-w-[1160px] px-5 py-20 sm:px-8 sm:py-28">
          <Reveal className="text-center">
            <span className="font-mono text-[11px] uppercase tracking-[0.18em] text-brand">{tx("Tarifs")}</span>
            {freePlan ? (
              <>
                <h2 className="mx-auto mt-4 max-w-[640px] font-archivo text-[clamp(26px,4.5vw,42px)] font-extrabold leading-[1.08] tracking-[-0.025em]">{tx("Ton premier client est")} <span className="text-brand">{tx("offert")}</span></h2>
                <p className="mx-auto mt-4 max-w-[52ch] text-[16px] leading-[1.6] text-white/65">{tx("Lance ton activité sans rien payer. Tu passes à une formule seulement quand tu accueilles ton deuxième client.")}</p>
              </>
            ) : (
              <>
                <h2 className="mx-auto mt-4 max-w-[640px] font-archivo text-[clamp(26px,4.5vw,42px)] font-extrabold leading-[1.08] tracking-[-0.025em]">{tx("Une formule pour")} <span className="text-brand">{tx("chaque taille")}</span></h2>
                <p className="mx-auto mt-4 max-w-[52ch] text-[16px] leading-[1.6] text-white/65">{tx("Choisis le nombre de clients que tu veux suivre. Tu changes de formule quand tu grandis.")}</p>
              </>
            )}
            <PricingSwitch tone="dark" className="mt-6" />
          </Reveal>

          {plans.length > 0 || freePlan ? (
            <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {freePlan ? <Reveal><FreePlanCard plan={freePlan} href={signup} tone="dark" className="h-full" /></Reveal> : null}
              {plans.map((p, i) => (
                <Reveal key={p.id} delay={(i + (freePlan ? 1 : 0)) * 80} className="flex flex-col gap-4 rounded-[24px] border border-white/12 bg-gradient-to-b from-white/[0.05] to-white/[0.01] p-7 transition-all hover:-translate-y-1 hover:border-brand/40">
                  <div className="font-archivo text-[20px] font-bold">{tx(p.name)}</div>
                  <div><PlanPrice plan={p} className="font-archivo text-[30px] font-extrabold tracking-[-0.02em] text-brand" subClassName="text-[12.5px] text-white/50" /></div>
                  <div className="text-[14px] text-white/70">{p.client_limit == null ? tx("Clients illimités") : `${tx("Jusqu'à")} ${p.client_limit} ${p.client_limit > 1 ? tx("clients actifs") : tx("client actif")}`}</div>
                  {p.setup_fee_cents > 0 ? <div className="text-[12.5px] text-white/45">+ {formatEuros(p.setup_fee_cents)} {tx("de mise en place (une fois)")}</div> : null}
            ...
            <Reveal className="mx-auto mt-12 max-w-[480px] rounded-[24px] border border-brand/25 bg-gradient-to-b from-brand/[0.12] to-transparent p-9 text-center">
              ...
              <div className="mt-4 font-archivo text-[26px] font-extrabold">{tx("Premier client offert")}</div>
              <p className="mt-2 text-[14.5px] leading-[1.6] text-white/65">{tx("Crée ton espace et démarre gratuitement.")} {reseller.name} {tx("te proposera ses formules dès que tu grandis.")}</p>
      ...
      <section id="faq" className="relative z-10 mx-auto w-full max-w-[820px] px-5 py-20 sm:px-8 sm:py-28">
        <Reveal className="text-center">
          ...
          <h2 className="mt-4 font-archivo text-[clamp(26px,4.5vw,40px)] font-extrabold tracking-[-0.025em]">{tx("Tout ce que tu te demandes")}</h2>
        </Reveal>
      ...
      <section className="relative z-10 mx-auto w-full max-w-[1160px] px-5 pb-24 sm:px-8">
        <Reveal className="relative overflow-hidden rounded-[28px] border border-white/12 p-10 text-center sm:p-16">
          ...
          <div className="relative">
            <h2 className="mx-auto max-w-[680px] font-archivo text-[clamp(28px,5vw,46px)] font-extrabold leading-[1.05] tracking-[-0.03em]">{tx("Ton business de coaching commence maintenant")}</h2>
            <p className="mx-auto mt-4 max-w-[52ch] text-[16px] leading-[1.6] text-white/70">{tx("Ton premier client est offert. Aucune carte requise. Sois en ligne dans 5 minutes.")}</p>
      ...
      <footer className="relative z-10 border-t border-white/10">
        <div className="mx-auto flex w-full max-w-[1160px] flex-col items-center gap-2 px-5 py-10 text-center sm:px-8">
          ...
          <p className="text-[12.5px] text-white/45">{tx("Plateforme de coaching en marque blanche, propulsée par l'IA. Premier client offert, sans engagement.")}</p>
        </div>
      </footer>
